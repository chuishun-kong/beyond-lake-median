...
|---|---|---|---|
| H1 calibration error and H3 within-target ordering | `capacity_boundary/` | `run_nested_capacity_boundary.py`, `run_h3_paired_source_audit_nested.py` | Repeat-level H1 metrics and H3 summaries, including full precision |
...
are not bundled, so its summary alone cannot reproduce paired rank effects.
...
table-generation script is not standalone. The final script and JSON outputs
for the post hoc H1 three-learner multiplicity sensitivity (Table S6E) are not
